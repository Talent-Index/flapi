define([
  ...
  var GameEngine = declare(null, {
    ...
    loop: function () {
      if (!this.running) return; var now = performance.now(); var dt = (now - this.time.last) / 1000; this.time.last = now; this.time.acc += dt; var step = this.time.step;
    ...
    update: function (dt) {
      ...
      for (var i = this.hearts.length - 1; i >= 0; i--) {
        ...
        if (!heart.collected) {
          // Move heart left at same speed as pipes
          ...
          if (heart.x < playerAABB.x + playerAABB.w &&
            ...
            this.lives++;
            this.hud.setLives(this.lives);
            this.audio.playPickup(); // Play sound if available
          }

          // Remove hearts that have moved off screen
    ...
    drawHearts: function (ctx) {
      ...
      for (var i = 0; i < this.hearts.length; i++) {
        ...
        var floatOffset = Math.sin(time + heart.x * 0.01) * 2;
        var currentY = heart.y + floatOffset;

        // Pulsing effect
        var pulseScale = 1 + Math.sin(time * 3 + heart.x * 0.02) * 0.1;

        ctx.globalAlpha = 0.9;
        ctx.translate(heart.x, currentY);
        ...
        // Add sparkle effect
        var sparkleTime = time * 4;
        for (var j = 0; j < 3; j++) {
          var sparkleX = Math.cos(sparkleTime + j * Math.PI * 0.66) * (size + 5);
          var sparkleY = Math.sin(sparkleTime + j * Math.PI * 0.66) * (size + 5);
          var sparkleAlpha = (Math.sin(sparkleTime * 2 + j) + 1) * 0.3;
